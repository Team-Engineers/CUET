...
import freeicon from "../../assets/images/courses/freeicon.png"
    ...
    <div className="flex justify-end items-center w-[90%]">
      {index === 0 || (index <= 4 && auth.user) ? (
        ...
        <Link to={`/login`} className="btn hover:bg-[#34A853] bg-[#34A853] text-white flex justify-center rounded-full items-center text-lg font-bold min-h-0 h-10 px-8 my-3">
          {auth.user ? "Practice" : "Login to Practice"}
        </Link>
      )}
    </div>
  </div>
  ...
  return (
    <div className=" rounded-3xl " style={{ background: bgcolor }} >
      ...
      {index === 0 && (
        ...
        <Link to={`/login`}><button className="btn hover:bg-[#34A853] my-3 bg-[#34A853] rounded-full text-white min-h-0 h-8 px-8">
        {auth.user ? "Take Test" : "Login "}
        </button>
        </Link>
      )}
        </div>
