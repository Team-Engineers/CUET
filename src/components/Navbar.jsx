...
const Navbar = () => {
  ...
  return (
    <div
      ...
      } w-full 2xl:max-w-[82vw] min-h-[10vh]`}
    >
        ...
        <div className="flex flex-row items-center justify-start gap-[7px]">
          <NavLink to={"/"} className="no-underline gap-2 items-center flex">
            <img
              ...
              src={"/logo.png"}
            />
            <b className="relative text-blueviolet-100 text-xl">
              CUET-TestKnock
            </b>
          </NavLink>
        </div>
      ...
      <div className="navbar-end mr-2  lg:ml-auto w-full relative">
          ...
          {nav_buttons.map((nav, i) => (
            <li className="hidden lg:flex ">
              <div className="h-[21px] flex flex-col items-start justify-start gap-[2px] text-blueviolet-100">
                ...
                >
                  <b className="flex-1 relative cursor-pointer text-blueviolet-100 text-lg font-semibold">
                    {nav.title}
                  </b>
                </NavLink>
        ...
        <div className="dropdown">
          <div
            tabIndex={0}
            role="button"
            className="btn btn-ghost lg:hidden p-0 mr-5"
          >
            <GiHamburgerMenu className="text-blueviolet-100" size={"2em"} />
          </div>
          <ul
            tabIndex={0}
            className="menu menu-sm dropdown-content mt-3 z-[1] p-2 shadow bg-base-100 rounded-box w-52 right-[0px]"
          >
            {nav_buttons.map((nav, i) => (
              <li className="my-1">
                <div className="h-[21px] flex flex-col items-start justify-start gap-[2px] text-blueviolet-100 ">
                  <NavLink
                    to={nav.path}
                    className={`no-underline text-blueviolet-100 text-center w-full ${
                      location.pathname === nav.path ? "active" : ""
                    }`}
                  >
            ...
            <li className="my-1">
              <div className="h-[21px] flex flex-col items-start justify-start gap-[2px] text-blueviolet-100 ">
                <NavLink
                  ...
                  className={`no-underline text-blueviolet-100 text-center w-full `}
                >
                  <b className="flex-1 relative cursor-pointer text-salmon-200 text-lg md:font-bold">
                    Signup
                  </b>
                </NavLink>
              </div>
        ...
        <b className="hidden lg:block absolute text-blueviolet-100 top-20 mr-9">
          +91 8279662680
        </b>
      </div>
    </div>
